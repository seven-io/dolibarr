...
class SMS_SMSOutbox_Setting extends SevenBaseSettingController {
	...
	public function post_request_handler() {
		...
		if (GETPOST('action') == 'clear_sms_outbox') {
			if (!$user->rights->seven->permission->delete) accessforbidden();
			if ($this->clear_sms_outbox()) $this->addNotificationMessage('SMS Outbox cleared');
			else $this->addNotificationMessage('Failed to clear SMS Outbox', 'error');
		}
	}

	private function clear_sms_outbox(): bool {
	...
	public function render() {
		...
		if (isset($_GET['pageno'])) $current_page = filter_var($_GET['pageno'], FILTER_SANITIZE_NUMBER_INT);
		...
		print dol_get_fiche_head(sevenAdminPrepareHead(), $this->context, $langs->trans($this->page_name), -1);

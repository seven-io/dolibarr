...
class SMS_SupplierOrder_Setting extends SevenBaseSettingController {
	...
	public $db_key = "SEVEN_SUPPLIER_ORDER_SETTING";
	...
	function __construct($db) {
		...
		$this->errors = [];
		$this->context = 'supplier_order';
		$this->page_name = 'supplier_order_page_title';
	}
		...
		if (!empty($_POST)) {
			...
			if ($action == "update_{$this->context}") {
				...
				if (count($this->errors) > 0) {
					$this->add_notification_message("Failed to save supplier order settings", "error");
				} else {
					$this->add_notification_message("Supplier Order settings saved");
				}
			}
		}
	}
	...
	public function trigger_sms(CommandeFournisseur $object, $action) {
		global $db;
	...
	protected function replace_keywords_with_value(CommandeFournisseur $object, $message) {
		global $db;

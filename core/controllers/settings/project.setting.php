...
class SMS_Project_Setting extends SevenBaseSettingController {
	...
	public function trigger_send_sms(Project $object): void {
		global $db;

		$settings = $this->getSettings();
		if ($settings->enable != 'on') return;

		$leadStatus = $this->get_lead_status($object->opp_status);
	...
	private function get_lead_status($opp_status): string {
		return match ($opp_status) {
			1 => 'prospection',
			2 => 'qualification',
			3 => 'proposal',
			4 => 'negotiation',
	...
	protected function fillKeywordsWithValues(Project $object, $message) {
		global $db;

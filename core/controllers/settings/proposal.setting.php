...
class SMS_Proposal_Setting extends SevenBaseSettingController {
		...
		if (!empty($_POST)) {
			...
			if ($action == "update_{$this->context}") {
				...
				if (count($this->errors) > 0) {
					$this->add_notification_message("Failed to save proposal settings", "error");
				} else {
					$this->add_notification_message("Proposal settings saved");
				}
			}
		}
	}
	...
	public function trigger_sms(Propal $object, $action) {
		global $db;
	...
	public function replace_keywords_with_value(Propal $object, $message) {
		global $db;
	...
	public function render() {
		...
		print dol_get_fiche_head($head, 'propal', $langs->trans($this->page_name), -1);

...
class SMS_Proposal_Setting extends SevenBaseSettingController {
	var $context;
	var $db;
	var $errors;
	var $form;
	var $log;
	var $page_name;
	public $db_key = 'SEVEN_PROPAL_SETTING';
	...
	function __construct($db) {
		...
		$this->page_name = 'propal_page_title';
	}
	...
	private function getDefaultSettings(): string {
		return json_encode([
			...
			'sms_templates' => [
				'created' => 'Hi [company_name], your proposal has been created!',
				'validated' => 'Hi [company_name], here\'s our proposal as spoken earlier.',
				'updated' => 'Hi [company_name], here\'s our updated proposal.',
				'close_signed' => 'Hi [company_name], thank you for signing the proposal. We are looking forward to serve you!',
				'close_refused' => 'Hi [company_name], your proposal has been refused.',
			],
		]);
	}
	...
	public function updateSettings(): void {
		...
		$settings = json_encode([
			...
			'send_on' => [
				...
				'close_signed' => GETPOST('send_on_close_signed'),
				'close_refused' => GETPOST('send_on_close_refused'),
			],
			'sms_templates' => [
				'created' => GETPOST('sms_templates_created'),
				'validated' => GETPOST('sms_templates_validated'),
				'updated' => GETPOST('sms_templates_updated'),
				'close_signed' => GETPOST('sms_templates_close_signed'),
				'close_refused' => GETPOST('sms_templates_close_refused'),
			],
		]);

		$error = dolibarr_set_const($db, $this->db_key, $settings);
		if ($error < 1) {
			$this->log->add('Seven', 'failed to update the proposal settings: ' . print_r($settings, 1));
			$this->errors[] = 'There was an error saving proposal settings.';
		}
		if (count($this->errors) > 0) $this->addNotificationMessage('Failed to save proposal settings', 'error');
		else $this->addNotificationMessage('Proposal settings saved');
	}
	...
	public function trigger_sms(Propal $object, $action): void {
		global $db;

		$settings = $this->getSettings();
		if ($settings->enable != 'on') return;
		$this->log->add('Seven', 'propal_trigger_sms');
		...
		$this->log->add('Seven', 'SMS Successfully sent from fx propal_trigger_sms');
	}

	public function fillKeywordsWithValues(Propal $object, $message) {
		global $db;
		...
		$keywords = [
			...
			'[ref]' => !empty($object->newref) ? $object->newref : '',
			'[date_of_proposal]' => !empty($object->date) ? date('m/d/Y', $object->date) : '',
			'[delivery_date]' => !empty($object->delivery_date) ? date('m/d/Y', $object->delivery_date) : '',
			'[availability_delay]' => !empty($object->availability) ? $object->availability : '',
			'[payment_terms]' => !empty($object->cond_reglement_doc) ? $object->cond_reglement_doc : '',
			'[source]' => !empty($object->demand_reason) ? $object->demand_reason : '',
			'[total]' => !empty($object->total_ttc) ? $object->total_ttc : '',
	...
	public function render() {
		...
		print dol_get_fiche_head(sevenAdminPrepareHead(), 'propal', $langs->trans($this->page_name), -1);
		...
		<script>
			const entity_keywords = <?= json_encode($this->getKeywords()); ?>;
			jQuery(function ($) {

...
class modSeven extends DolibarrModules {
	...
	public function __construct($db) {
		...
		// Prerequisites
		$this->phpmin = [8, 0]; // Minimum version of PHP required by module
	...
	public function init($options = '') {
		...
		foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
			...
			if ($myTmpObjectArray['includerefgeneration']) {
				...
				$sql = array_merge($sql, [
					sprintf('DELETE FROM %sdocument_model WHERE nom = \'standard_%s\' AND type = \'%s\' AND entity = %d', MAIN_DB_PREFIX, strtolower($myTmpObjectKey), $this->db->escape(strtolower($myTmpObjectKey)), (int)$conf->entity),
					sprintf('INSERT INTO %sdocument_model (nom, type, entity) VALUES(\'standard_%s\', \'%s\', %d)', MAIN_DB_PREFIX, strtolower($myTmpObjectKey), $this->db->escape(strtolower($myTmpObjectKey)), (int)$conf->entity),
					sprintf('DELETE FROM %sdocument_model WHERE nom = \'generic_%s_odt\' AND type = \'%s\' AND entity = %d', MAIN_DB_PREFIX, strtolower($myTmpObjectKey), $this->db->escape(strtolower($myTmpObjectKey)), (int)$conf->entity),
					sprintf('INSERT INTO %sdocument_model (nom, type, entity) VALUES(\'generic_%s_odt\', \'%s\', %d)', MAIN_DB_PREFIX, strtolower($myTmpObjectKey), $this->db->escape(strtolower($myTmpObjectKey)), (int)$conf->entity)
				]);
			}
		}
